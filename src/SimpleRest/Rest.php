...
class Rest {
	...
	private function processRequest() {
		$this->request['resource'] = (isset($_GET['rewrite']) && !empty($_GET['rewrite'])) ? $_GET['rewrite'] : 'index';
		unset($_GET['rewrite']);
	...
	public function process() {
		try	{
			...
			$this->response = array('response' => array('error' => array('code' => $re->getCode(), 'message' => $re->getMessage())));
		}
		$this->response()->send();
	}
	...
	private function getController() {
		$expected = str_replace("/", "", $this->request['resource']);
	...
	*/
	private function getResponseContentType($type = null) {
		return (isset(self::$contentTypes[$type])) ? self::$contentTypes[$type] : "text/plain";
	}
